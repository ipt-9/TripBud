      ...
      <div class="triporganizer-content">
        ...
        <div class="triporganizer-card">
          <div v-for="(trip, index) in trips" :key="index" class="trip-item" @click="viewTrip(trip)">
            <img v-for="(img, index) in trashImages" :key="index" :src="img" class="delete-icon" @click.stop="deleteTrip(index)"/>
            <span>{{ trip.name }}</span>
          </div>
        </div>
        <button type="submit" class="newtrip-button">Create New Trip</button>
      </div>
    </div>
</template>
